...
//! in-country S3/MinIO later. Same shape as `services/auth/src/store.rs` and
//! `services/places/src/store.rs` (kept service-local rather than shared —
//! nothing else shares it either).
