//! Document storage behind a trait so the local dev backend can be swapped for
//! in-country S3/MinIO later. KYC bytes must be kept in Nepal and encrypted at
//! rest in production (AGENTS.md golden rule #6) — the local backend below is
