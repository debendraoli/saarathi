...
One queue, a `type` tag. Each driver declares **exactly one** job type they'll take — ride or delivery, never both — **at KYC, stored on the driver record** (`service_types`), correctable by Ops from the dashboard ([05 §5.5.1](05-technical-architecture.md)). "Cross-vertical dispatch is the whole point" describes the *engine and the fleet*: one dispatch pool serves both verticals, so a rider-heavy afternoon and a food-order-heavy evening both draw from the same driver base — just from different drivers within it, not the same driver flexing between both. Prep-time gating for food; **batching** (2 nearby orders) is a Phase-4 optimization once density exists ([04 §2](04-operational-procedures.md)).

The filter is strict in both directions: a **delivery** driver never receives a ride request, and a **ride** driver never receives a merchant order. Merchant orders reach couriers by being `delivery`-typed trips on this same queue — there is no second dispatcher. The same job-type filter also governs the availability probes (see [04 §2](04-operational-procedures.md)), so "no couriers nearby" means no *delivery* driver nearby, not merely no driver.

> ⚠️ **Supply-side watch item:** because each driver is single-vertical, the ride/delivery split of the *fleet* directly sets delivery's ceiling — there's no slack from ride drivers picking up food jobs in a lull. In a market this thin, a fleet that's mostly ride drivers would starve delivery of couriers well before demand is the problem. Track the ride-vs-delivery driver split from launch ([04 §7](04-operational-procedures.md)) and steer walk-in intake toward whichever side is thin, treating a persistent imbalance as a supply incident, not a reporting curiosity.
